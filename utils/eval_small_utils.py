...
from utils.utils import (
    stop_token_list,
    prompt_template,
    prompt_template_medAlpaca,
    prompt_template_MMedLM
)
...
def performance_eval(config, prompts, answers, file_path):
    ...
    llm = LLM(
        ...
        dtype='bfloat16',
        # Special Notice
        # Due to the deadlock issue in distributed inference of vLLM
        # We will only use 1 GPU for inference of 2B/7B/8B models
        # Link: https://github.com/vllm-project/vllm/issues/3455
        #
        # For 70B and 8x7B MoE models, we have to use multiple GPUs for inference
        # We have a solution and please check the corresponding inference codes
        # Acknowledgement: Benjamin Kitor
        # Link: https://github.com/vllm-project/vllm/issues/2794
        # Please use `tensor_parallel_size > 1` for faster inference if it works on your side.
...
def medqa_eval(config, tokenizer, file_path, data_path):
    """
...
def pubmedqa_eval(config, tokenizer, file_path, data_path):
    """
...
def medmcqa_eval(config, tokenizer, file_path, data_path):
    """
...
def usmle_eval(config, tokenizer, file_path, data_path):
    """
...
def medexpqa_eval(config, tokenizer, file_path, data_path):
    """
...
def mmlu_eval(config, tokenizer, file_path, data_path):
    """
...
def mcmle_eval(config, tokenizer, file_path, data_path):
    """
...
def cmmlu_eval(config, tokenizer, file_path, data_path):
    """
...
def frenchmedmcqa_eval(config, tokenizer, file_path, data_path):
    """
...
def headqa_eval(config, tokenizer, file_path, data_path):
    """
